...
<a class="flex items-center gap-3 px-4 py-3 text-on-surface-variant hover:bg-surface-container-high transition-all rounded-lg font-label-md text-label-md" href="<?php echo e(url('/dashboard')); ?>">
<span class="material-symbols-outlined">dashboard</span> Dashboard
            </a>
<a class="flex items-center gap-3 px-4 py-3 text-on-surface-variant hover:bg-surface-container-high transition-all rounded-lg font-label-md text-label-md" href="<?php echo e(url('/upload-resep')); ?>">
<span class="material-symbols-outlined">description</span> Resep Saya
            </a>
<a class="flex items-center gap-3 px-4 py-3 bg-secondary-container text-on-secondary-container rounded-lg font-bold font-label-md text-label-md translate-x-1 transition-transform" href="<?php echo e(url('/riwayat-pesanan')); ?>">
<span class="material-symbols-outlined" style="font-variation-settings: 'FILL' 1;">receipt_long</span> Riwayat Pesanan
            </a>
<a class="flex items-center gap-3 px-4 py-3 text-on-surface-variant hover:bg-surface-container-high transition-all rounded-lg font-label-md text-label-md" href="<?php echo e(url('/lokasi-apotek')); ?>">
<span class="material-symbols-outlined">location_on</span> Lokasi Apotek
            </a>
<a class="flex items-center gap-3 px-4 py-3 text-on-surface-variant hover:bg-surface-container-high transition-all rounded-lg font-label-md text-label-md" href="<?php echo e(url('/profil')); ?>">
...
<div>
<p class="font-label-md text-label-md text-primary mb-1">Pesanan #ORD-<?php echo e(str_pad($pesananAktif->id, 4, '0', STR_PAD_LEFT)); ?></p>
<h3 class="font-headline-md text-headline-md text-on-surface">Estimasi Selesai: Hari Ini</h3>
</div>
<span class="bg-secondary-container text-on-secondary-container px-4 py-2 rounded-full font-label-md text-label-md font-bold"><?php echo e($pesananAktif->status); ?></span>
</div>
<!-- Progress Timeline -->
...
<p class="font-label-sm text-label-sm text-on-surface-variant/70"><?php echo e(\Carbon\Carbon::parse($pesananAktif->tanggal_pesanan)->format('d M Y')); ?></p>
</div>
</div>
<!-- Step 2 -->
<div class="flex flex-col items-center gap-3">
...
<div>
<p class="font-label-md text-label-md text-on-surface"><?php echo e($pesananAktif->detailPesanans->pluck('obat.nama_obat')->implode(', ')); ?> (<?php echo e($pesananAktif->detailPesanans->count()); ?> items)</p>
<p class="font-body-md text-body-md text-on-surface-variant">Total: Rp <?php echo e(number_format($pesananAktif->total_harga, 0, ',', '.')); ?></p>
</div>
</div>
</div>
</div>
...
<div class="space-y-4">
<?php $__empty_1 = true; $__currentLoopData = $pesananSelesai; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $pesanan): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
<!-- History Card -->
...
<div class="w-16 h-16 rounded-xl <?php echo e($pesanan->status == 'Dibatalkan' ? 'bg-error-container text-error' : 'bg-surface-container text-primary'); ?> flex items-center justify-center shrink-0">
<span class="material-symbols-outlined text-[32px]"><?php echo e($pesanan->status == 'Dibatalkan' ? 'cancel' : 'check_circle'); ?></span>
</div>
<div class="flex-1">
<div class="flex items-center gap-3 mb-1">
<h3 class="font-headline-md text-headline-md text-on-surface">Pesanan #ORD-<?php echo e(str_pad($pesanan->id, 4, '0', STR_PAD_LEFT)); ?></h3>
<span class="<?php echo e($pesanan->status == 'Dibatalkan' ? 'bg-error-container text-error' : 'bg-surface-container-high text-on-surface-variant'); ?> px-2 py-0.5 rounded text-label-sm"><?php echo e($pesanan->status); ?></span>
</div>
<p class="font-body-md text-body-md text-on-surface-variant mb-2"><?php echo e(\Carbon\Carbon::parse($pesanan->tanggal_pesanan)->format('d M Y')); ?> • <?php echo e($pesanan->detailPesanans->count()); ?> Produk</p>
<p class="font-label-md text-label-md text-on-surface line-clamp-1"><?php echo e($pesanan->detailPesanans->pluck('obat.nama_obat')->implode(', ')); ?></p>
</div>
<div class="md:text-right flex flex-col items-start md:items-end gap-3 mt-4 md:mt-0">
<p class="font-headline-md text-headline-md text-primary">Rp <?php echo e(number_format($pesanan->total_harga, 0, ',', '.')); ?></p>
...
<!-- Pagination -->
<?php if($pesananSelesai->hasPages()): ?>
<div class="mt-8 flex justify-center">
    <?php echo e($pesananSelesai->links()); ?>
...
<?php echo $__env->make('layouts.sipa', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Nada PAW1\tugas_uas\resources\views/riwayat-pesanan.blade.php ENDPATH**/ ?>