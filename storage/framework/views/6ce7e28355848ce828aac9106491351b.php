...
<!-- SideNavBar -->
<aside class="hidden lg:flex flex-col gap-2 p-4 w-72 sticky top-24 h-[calc(100vh-64px)] border-r border-outline-variant bg-surface-container-low">
<div class="mb-8 px-4">
<h2 class="text-[20px] font-bold text-primary">Panel Farmasi</h2>
<p class="text-[12px] font-semibold text-on-surface-variant uppercase tracking-wider">Portal Pelanggan</p>
</div>
<nav class="flex flex-col gap-1">
<a class="flex items-center gap-4 text-on-surface-variant p-3 hover:bg-surface-container-high rounded-xl transition-all" href="<?php echo e(url('/katalog')); ?>">
<span class="material-symbols-outlined">vaccines</span>
<span class="font-semibold text-[14px]">Cari Obat</span>
</a>
<a class="flex items-center gap-4 text-on-surface-variant p-3 hover:bg-surface-container-high rounded-xl transition-all" href="<?php echo e(url('/upload-resep')); ?>">
<span class="material-symbols-outlined">description</span>
<span class="font-semibold text-[14px]">Resep Saya</span>
</a>
<a class="flex items-center gap-4 text-on-surface-variant p-3 hover:bg-surface-container-high rounded-xl transition-all" href="<?php echo e(url('/riwayat-pesanan')); ?>">
<span class="material-symbols-outlined">receipt_long</span>
<span class="font-semibold text-[14px]">Riwayat Transaksi</span>
</a>
<a class="flex items-center gap-4 bg-primary text-on-primary rounded-xl p-3 shadow-md" href="<?php echo e(url('/profil')); ?>">
<span class="material-symbols-outlined">person</span>
<span class="font-semibold text-[14px]">Pengaturan Akun</span>
</a>
<a class="flex items-center gap-4 text-on-surface-variant p-3 hover:bg-surface-container-high rounded-xl transition-all" href="<?php echo e(url('/lokasi-apotek')); ?>">
<span class="material-symbols-outlined">location_on</span>
<span class="font-semibold text-[14px]">Lokasi Apotek</span>
</a>
</nav>
<div class="mt-auto p-4 bg-primary-container/20 rounded-2xl border border-primary-container/30">
<p class="text-[12px] font-bold text-primary mb-2">Punya resep dokter?</p>
<a href="<?php echo e(url('/upload-resep')); ?>" class="w-full py-3 bg-primary text-on-primary rounded-xl font-bold text-[14px] shadow-sm hover:opacity-90 transition-all flex justify-center text-center">
                Unggah Resep
            </a>
</div>
</aside>
...
<?php
    $userId = auth()->id() ?? 1;
?>
<?php if(file_exists(public_path('profiles/user_' . $userId . '.jpg'))): ?>
    <img src="<?php echo e(asset('profiles/user_' . $userId . '.jpg')); ?>?v=<?php echo e(time()); ?>" alt="Profile" class="w-16 h-16 rounded-full object-cover border-2 border-primary-fixed premium-shadow">
<?php else: ?>
...
<a href="<?php echo e(route('profile.edit')); ?>" class="text-primary font-bold text-[14px] px-4 py-2 hover:bg-primary/5 rounded-lg transition-all inline-block">Ubah Profil</a>
</div>
...
<p class="text-[16px] font-semibold text-on-surface"><?php echo e($pelanggan ? $pelanggan->nama : 'Pengguna Tamu'); ?></p>
</div>
<div class="space-y-1">
<label class="text-[12px] font-bold text-on-surface-variant uppercase tracking-wide">Email</label>
<p class="text-[16px] font-semibold text-on-surface"><?php echo e($pelanggan ? $pelanggan->email : '-'); ?></p>
</div>
<div class="space-y-1">
<label class="text-[12px] font-bold text-on-surface-variant uppercase tracking-wide">No. Telp</label>
<p class="text-[16px] font-semibold text-on-surface"><?php echo e($pelanggan ? $pelanggan->no_telp : '-'); ?></p>
</div>
<div class="space-y-1">
<label class="text-[12px] font-bold text-on-surface-variant uppercase tracking-wide">Member Sejak</label>
<p class="text-[16px] font-semibold text-on-surface"><?php echo e($pelanggan ? \Carbon\Carbon::parse($pelanggan->created_at)->format('d F Y') : '-'); ?></p>
</div>
</div>
</div>
<!-- Quick Stats -->
...
<p class="text-[24px] font-extrabold text-on-surface"><?php echo e($totalResepAktif ?? 0); ?></p>
</div>
...
<p class="text-[14px] text-on-surface-variant leading-relaxed">
                                    <?php echo e($pelanggan ? $pelanggan->alamat : 'Belum ada alamat'); ?>
...
<a href="<?php echo e(route('profile.edit')); ?>" class="px-3 py-1.5 border border-outline-variant rounded-lg text-[12px] font-bold hover:bg-surface-container transition-colors inline-block">Ubah</a>
</div>
<div class="flex items-center justify-between">
<div class="flex items-center gap-3">
...
<div id="toggle-pesanan" class="w-10 h-5 <?php echo e(isset($preferences) && $preferences['notif_pesanan'] ? 'bg-primary' : 'bg-outline-variant'); ?> rounded-full relative cursor-pointer toggle-switch" data-active="<?php echo e(isset($preferences) && $preferences['notif_pesanan'] ? 'true' : 'false'); ?>">
<div class="absolute <?php echo e(isset($preferences) && $preferences['notif_pesanan'] ? 'right-1' : 'left-1'); ?> top-1 w-3 h-3 bg-white rounded-full toggle-knob transition-all"></div>
</div>
</div>
<div class="flex items-center justify-between">
<div class="flex items-center gap-3">
...
<div id="toggle-obat" class="w-10 h-5 <?php echo e(isset($preferences) && $preferences['notif_obat'] ? 'bg-primary' : 'bg-outline-variant'); ?> rounded-full relative cursor-pointer toggle-switch" data-active="<?php echo e(isset($preferences) && $preferences['notif_obat'] ? 'true' : 'false'); ?>">
<div class="absolute <?php echo e(isset($preferences) && $preferences['notif_obat'] ? 'right-1' : 'left-1'); ?> top-1 w-3 h-3 bg-white rounded-full toggle-knob transition-all"></div>
</div>
</div>
</div>
<?php if(auth()->check()): ?>
...
<div class="pt-6 mt-6 border-t border-outline-variant flex gap-4">
    <a href="<?php echo e(route('login')); ?>" class="flex-1 py-3 bg-surface-container-high text-on-surface border border-outline-variant rounded-xl font-bold text-[14px] hover:bg-surface-container-highest transition-all flex justify-center items-center gap-2">
        <span class="material-symbols-outlined text-[20px]">login</span>
        Masuk
    </a>
    <a href="<?php echo e(route('register')); ?>" class="flex-1 py-3 bg-primary text-on-primary rounded-xl font-bold text-[14px] hover:opacity-90 transition-all flex justify-center items-center gap-2 shadow-md shadow-primary/20">
...
<div id="modal-alamat" class="fixed inset-0 z-[100] hidden flex items-center justify-center bg-black/50 backdrop-blur-sm transition-opacity">
    <div class="bg-surface border border-outline-variant rounded-2xl w-full max-w-md p-6 shadow-2xl animate-fade-in-up">
        ...
        <form action="<?php echo e(route('profil.alamat')); ?>" method="POST">
            <?php echo csrf_field(); ?>
            <div class="mb-6">
                <label for="alamat" class="block text-sm font-bold text-on-surface mb-2">Alamat Lengkap</label>
                <textarea id="alamat" name="alamat" rows="4" class="w-full px-4 py-3 bg-surface-container-lowest border border-outline-variant rounded-xl focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all resize-none text-on-surface" placeholder="Masukkan alamat lengkap rumah Anda..." required><?php echo e($pelanggan ? $pelanggan->alamat : ''); ?></textarea>
            </div>
...
<script>
    ...
    document.querySelectorAll('.toggle-switch').forEach(toggle => {
        toggle.addEventListener('click', function() {
            ...
            fetch("<?php echo e(route('profil.preferences')); ?>", {
                method: 'POST',
                headers: {
...
<?php echo $__env->make('layouts.sipa', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Nada PAW1\tugas_uas\resources\views/profil.blade.php ENDPATH**/ ?>