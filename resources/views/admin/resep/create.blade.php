        ...
        <div class="col-lg-7">
            <div class="card shadow-sm border-0">
                <div class="card-header bg-white border-bottom">
                    <h3 class="card-title fw-bold text-primary m-0"><i class="bi bi-file-medical me-2"></i>Tambah Resep</h3>
                </div>
                <div class="card-body">
                    @if ($errors->any())
                    ...
                    <form action="{{ route('resep.store') }}" method="POST">
                        ...
                        <div class="row g-3">
                                ...
                                <select name="pelanggan_id" class="form-select" required>
                                    ...
                                    @foreach ($pelanggan as $item)
                                        <option value="{{ $item->id }}"
                                            {{ old('pelanggan_id') == $item->id ? 'selected' : '' }}>{{ $item->nama }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-6">
                            ...
                            <div class="col-12">
                                <label class="form-label fw-semibold">File Resep <small class="text-muted">(nama
                                        file)</small></label>
                                <input type="text" name="file_resep" class="form-control" value="{{ old('file_resep') }}"
                                    placeholder="contoh: resep_pasien.jpg" required>
                            </div>
                            <div class="col-12">
