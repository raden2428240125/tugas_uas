        ...
        <div class="col-lg-7">
            <div class="card shadow-sm border-0">
                <div class="card-header bg-white border-bottom">
                    <h3 class="card-title fw-bold text-primary m-0"><i class="bi bi-credit-card me-2"></i>Tambah Pembayaran
                    </h3>
                </div>
                <div class="card-body">
                    @if ($errors->any())
                                ...
                                <select name="pesanan_id" class="form-select" required>
                                    ...
                                    @foreach ($pesanan as $item)
                                        <option value="{{ $item->id }}"
                                            {{ old('pesanan_id') == $item->id ? 'selected' : '' }}>
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="metode_pembayaran" class="form-select" required>
                                    @foreach (['QRIS', 'Transfer Bank', 'E-Wallet', 'COD'] as $m)
                                        <option value="{{ $m }}"
                                            {{ old('metode_pembayaran') == $m ? 'selected' : '' }}>{{ $m }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-6">
                                ...
                                <input type="number" name="jumlah_bayar" class="form-control"
                                    value="{{ old('jumlah_bayar') }}" min="0" required>
                            </div>
                            <div class="col-md-6">
                                ...
                                <select name="status_pembayaran" class="form-select" required>
                                    @foreach (['Belum Dibayar', 'Menunggu Verifikasi', 'Lunas'] as $s)
                                        <option value="{{ $s }}"
                                            {{ old('status_pembayaran') == $s ? 'selected' : '' }}>{{ $s }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-6">
