        ...
        <div class="col-lg-7">
            <div class="card shadow-sm border-0">
                <div class="card-header bg-white border-bottom">
                    <h3 class="card-title fw-bold text-primary m-0"><i class="bi bi-pencil-square me-2"></i>Edit Detail
                        Pesanan</h3>
                </div>
                <div class="card-body">
                    @if ($errors->any())
                        ...
                        <div class="row g-3">
                                ...
                                <select name="pesanan_id" class="form-select" required>
                                    @foreach ($pesanans as $pesanan)
                                        <option value="{{ $pesanan->id }}"
                                            {{ old('pesanan_id', $detailPesanan->pesanan_id) == $pesanan->id ? 'selected' : '' }}>
                                ...
                                <select name="obat_id" class="form-select" required>
                                    @foreach ($obats as $obat)
                                        <option value="{{ $obat->id }}"
                                            {{ old('obat_id', $detailPesanan->obat_id) == $obat->id ? 'selected' : '' }}>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="number" name="jumlah" class="form-control"
                                    value="{{ old('jumlah', $detailPesanan->jumlah) }}" min="1" required>
                            </div>
                        </div>
